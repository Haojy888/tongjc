...
# ──── HTML PAGE (Clean - No Emoji) ────
NEW_HTML = """<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Deep Love Master Chat Assistant</title>
<style>
  :root {
    --bg: #0a0a0a; --card: #141414; --card2: #1e1e1e; --border: #2a2a2a;
    --text: #e8e8e8; --text2: #888; --accent: #ff5e5e; --accent2: #ff3b3b;
    --green: #3dd68c; --yellow: #f5a623; --blue: #5b6cf0; --purple: #a855f7;
    --radius: 10px;
  }
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .avatar {
    ...
    background: linear-gradient(135deg, #ff5e5e, #f30);
    display: flex; align-items: center; justify-content: center;
    font-size: 18px; font-weight: bold; color: #fff;
  }
  .info h3 { font-size: 14px; }
  .info span { font-size: 11px; color: var(--text2); }
  .chat-messages {
  ...
  .msg.me {
    align-self: flex-end;
    background: linear-gradient(135deg, #ff5e5e, #d44);
  ...
  .analysis-panel::-webkit-scrollbar-thumb { background: #333; border-radius: 2px; }
  .panel-header {
    padding: 14px 18px; border-bottom: 1px solid var(--border);
  ...
  .signal-strong { background: rgba(61,214,140,0.12); color: var(--green); }
  .signal-medium { background: rgba(245,166,35,0.12); color: var(--yellow); }
  .signal-weak { background: rgba(255,94,94,0.12); color: var(--accent); }

  .commentary {
    background: linear-gradient(135deg, rgba(255,94,94,0.06), rgba(168,85,247,0.04));
    border: 1px solid rgba(255,94,94,0.15); border-radius: var(--radius);
    padding: 12px; font-size: 13px; line-height: 1.7; color: #ddd;
  }

  .indicator-list { list-style: none; display: flex; flex-direction: column; gap: 4px; }
  .indicator-list li {
    padding: 6px 10px; border-radius: 6px; font-size: 12px;
    display: flex; align-items: flex-start; gap: 6px;
  }
  .indicator-list li.good { background: rgba(61,214,140,0.06); color: var(--green); }
  .indicator-list li.warn { background: rgba(255,94,94,0.06); color: var(--accent); }
  ...
  .suggestion-card .tag {
    ...
    background: rgba(91,108,240,0.15); color: #9aa6ff;
  }
  .suggestion-card .source { font-size: 10px; color: var(--text2); }
  .suggestion-card .example {
    background: var(--card2); padding: 8px 12px; border-radius: 6px;
    font-size: 13px; margin: 6px 0; border-left: 2px solid var(--accent);
  }
  .suggestion-card .reason { font-size: 11px; color: var(--text2); line-height: 1.5; }
  ...
  .book-item .type-badge {
    font-size: 9px; padding: 1px 5px; border-radius: 4px;
    font-weight: 600; flex-shrink: 0; margin-top: 1px;
  }
  .type-principle { background: rgba(245,166,35,0.2); color: var(--yellow); }
  .type-advice { background: rgba(61,214,140,0.2); color: var(--green); }
  .type-framework { background: rgba(168,85,247,0.2); color: var(--purple); }
  .book-item .src { font-size: 10px; color: var(--text2); margin-top: 2px; }

  .gate-result {
    padding: 8px 12px; border-radius: 6px; font-size: 12px; margin-top: 6px;
  }
  .gate-pass { background: rgba(61,214,140,0.08); color: var(--green); }
  .gate-warn { background: rgba(245,166,35,0.08); color: var(--yellow); }
  .gate-fail { background: rgba(255,94,94,0.08); color: var(--accent); }
  ...
  .empty-state {
    ...
    color: var(--text2); text-align: center; padding: 30px; gap: 8px;
  }
  .empty-state h3 { color: var(--text); font-size: 14px; }
  .empty-state p { font-size: 12px; line-height: 1.6; }

  .my-reply-section { padding: 0 18px; }
  .my-reply-section textarea {
  ...
  .my-reply-section textarea:focus { border-color: var(--accent); }
  .my-reply-section .btn-row {
    display: flex; gap: 6px; margin-top: 6px;
  }
  .my-reply-section .btn-row button {
    padding: 6px 14px; border-radius: 14px; border: none;
    font-size: 12px; cursor: pointer; font-weight: 600;
  }
  .btn-gate { background: var(--blue); color: #fff; }
  .btn-gate:hover { opacity: 0.9; }
  .btn-clear { background: transparent; border: 1px solid var(--border); color: var(--text2); }

  .search-section { padding: 10px 18px; }
  .search-section input {
    width: 100%; padding: 8px 12px; border-radius: 16px;
    border: 1px solid var(--border); background: var(--card2);
    color: var(--text); font-size: 12px; outline: none;
  }
  .search-section input:focus { border-color: var(--purple); }

  .loading {
    display: flex; align-items: center; justify-content: center;
    padding: 20px; color: var(--text2); font-size: 12px;
  }

  @media (max-width: 768px) {
    body { flex-direction: column; }
    .analysis-panel { width: 100%; max-height: 45vh; min-width: 0; }
    .msg { max-width: 88%; }
  }
</style>
</head>
...
<div class="chat-panel">
  <div class="chat-header">
    ...
    <div class="info">
      <h3>Tong Jincheng (Jingchen) - Deep Love Master</h3>
      <span>200+ relationship classics loaded - Real-time chat coach</span>
    </div>
  </div>
  <div class="chat-messages" id="chatMessages">
    <div class="empty-state" id="emptyState">
      <h3>[*] Start Chatting</h3>
      <p>Paste her message below and press Enter to analyze<br>
      Knowledge base: Mystery, Juggler, David D, Devil's Guide, and 90+ more classics</p>
    </div>
  </div>
  <div class="chat-input-area">
    <input type="text" id="herInput" placeholder="Paste her message here, press Enter to analyze..."
           autocomplete="off" onkeydown="if(event.key==='Enter')analyzeMsg()">
    <button onclick="analyzeMsg()">Analyze</button>
  </div>
</div>
...
<div class="analysis-panel" id="analysisPanel">
  <div class="panel-header">
    ...
    <span style="font-size:11px;color:var(--text2);cursor:pointer" onclick="searchKB()" title="Search knowledge base">[?] Search KB</span>
  </div>
  <div id="analysisContent">
    <div class="empty-state">
      <h3>[+] Waiting for message</h3>
      <p>Paste her message to see:<br>
      Multi-dimension analysis + reply suggestions + book knowledge</p>
    </div>
  </div>
</div>
...
async function analyzeMsg() {
  ...
  try {
    const resp = await fetch('/analyze', {
      ...
      body: JSON.stringify({message: msg, context: context.slice(-5)})
    });
    const data = await resp.json();
    lastAnalysis = data;
    context.push({her: msg, time: Date.now()});
    renderAnalysis(data);
  } catch(e) {
    document.getElementById('analysisContent').innerHTML =
      '<div class="section"><p style="color:var(--accent);font-size:13px;">[ERROR] Cannot connect to server.<br>Make sure the server is running: python tjc-server.py</p></div>';
  }
}
...
function renderAnalysis(data) {
  ...
  html += '<div class="section">' +
    ...
    '<span class="signal-badge ' + data.signal.cls + '">' + data.signal.label + '</span>' +
    '<span style="font-size:11px;color:var(--text2);margin-left:6px;">Scenario: ' + data.scenario + '</span>' +
    '</div>';

  // Commentary
  html += '<div class="section">' +
    '<div class="section-title">[JINGCHEN SAYS]</div>' +
    '<div class="commentary">' + data.commentary + '</div>' +
    '</div>';

  // Indicators
  if (data.goodPoints.length > 0 || data.warnPoints.length > 0) {
    html += '<div class="section"><div class="section-title">[BREAKDOWN]</div><ul class="indicator-list">';
    data.goodPoints.forEach(function(p) { html += '<li class="good">[+] ' + p + '</li>'; });
    data.warnPoints.forEach(function(p) { html += '<li class="warn">[!] ' + p + '</li>'; });
    html += '</ul></div>';
  }

  // Suggestions
  if (data.suggestions.length > 0) {
    html += '<div class="section"><div class="section-title">[REPLY SUGGESTIONS]</div>';
    data.suggestions.forEach(function(s, i) {
      html += '<div class="suggestion-card">' +
        '<div class="tag-row">' +
          '<span class="tag">Option ' + (i+1) + ' - ' + s.tag + '</span>' +
          (s.source ? '<span class="source">From: ' + s.source + '</span>' : '') +
        '</div>' +
  ...
  // Book knowledge matches
  if (data.bookKnowledge && data.bookKnowledge.length > 0) {
    html += '<div class="section"><div class="section-title">[BOOK KNOWLEDGE MATCHES]</div>';
    data.bookKnowledge.forEach(function(bk) {
      var typeClass = bk.type === 'principle' ? 'type-principle' : bk.type === 'advice' ? 'type-advice' : 'type-framework';
      var typeLabel = bk.type === 'principle' ? 'Principle' : bk.type === 'advice' ? 'Advice' : 'Framework';
      html += '<div class="book-item">' +
        '<span class="type-badge ' + typeClass + '">' + typeLabel + '</span>' +
        '<div>' +
          '<div>' + bk.text + '</div>' +
          '<div class="src">Source: ' + bk.source + '</div>' +
        '</div>' +
      '</div>';
    });
    html += '</div>';
  ...
  // Your reply input
  html += '<div class="section my-reply-section">' +
    '<div class="section-title">[YOUR REPLY - Gate Check]</div>' +
    '<textarea id="myReplyInput" placeholder="Type your reply here..."></textarea>' +
    '<div class="btn-row">' +
      '<button class="btn-gate" onclick="gateMyReply()">Check & Send</button>' +
      '<button class="btn-clear" onclick="document.getElementById(\'myReplyInput\').value=\'\'">Clear</button>' +
    '</div>' +
  ...
  html += '<div class="quick-actions">' +
    '<button onclick="showFrameworks()">All Frameworks</button>' +
...
function gateMyReply() {
  var myReply = document.getElementById('myReplyInput').value.trim();
  var resultDiv = document.getElementById('gateResult');
  if (!myReply) { resultDiv.innerHTML = ''; return; }

  var issues = [];
  if (myReply.length > 200) {
    issues.push({level:'warn', text:'Reply is too long. Keep it concise - this is chat, not an essay.'});
  }
  var meCount = (myReply.match(/I /gi)||[]).length + (myReply.match(/I'/gi)||[]).length;
  if (meCount > 5) {
    issues.push({level:'warn', text:'Too many "I" statements. Ask about HER more.'});
  }
  var badKw = ['please please','i beg you','pretty please','i\'m begging'];
  if (badKw.some(function(k){ return myReply.toLowerCase().includes(k); })) {
    issues.push({level:'fail', text:'Desperate tone detected. Rewrite with confidence.'});
  }
  var openers = ['hey','hi','what\'s up','you there','you awake','wyd'];
  if (openers.some(function(k){ return myReply.trim().toLowerCase() === k; })) {
    issues.push({level:'warn', text:'Low-effort opener. Add something specific and contextual.'});
  }

  var html = '';
  if (issues.length === 0) {
    html = '<div class="gate-result gate-pass">[OK] Looks good. Send it. Remember: authenticity is the best game.</div>';
    addBubble(myReply, 'me');
    if (lastAnalysis) {
      if (context.length > 0) context[context.length-1].my = myReply;
    }
  } else {
    issues.forEach(function(iss) {
      var cls = iss.level === 'fail' ? 'gate-fail' : 'gate-warn';
      html += '<div class="gate-result ' + cls + '">' + iss.text + '</div>';
    });
  }
...
function clearChat() {
  context = [];
  lastAnalysis = null;
  document.getElementById('chatMessages').innerHTML = '<div class="empty-state"><h3>[*] Chat Cleared</h3><p>Start a new conversation</p></div>';
  document.getElementById('analysisContent').innerHTML = '<div class="empty-state"><h3>[+] Waiting for message</h3><p>Paste a message to begin analysis</p></div>';
  document.getElementById('herInput').focus();
}

function showFrameworks() {
  fetch('/frameworks').then(function(r){return r.json();}).then(function(data){
    var html = '<div class="section"><div class="section-title">[ALL FRAMEWORKS]</div>';
    data.frameworks.forEach(function(fw){
      html += '<div style="background:var(--card);border:1px solid var(--border);border-radius:var(--radius);padding:12px;margin-bottom:8px;">' +
        '<div style="font-weight:700;font-size:13px;margin-bottom:4px;">' + fw.name + '</div>' +
        '<div style="font-size:10px;color:var(--text2);margin-bottom:6px;">Source: ' + fw.source + '</div>';
      fw.key_principles.forEach(function(p){
        html += '<div style="font-size:12px;color:#ccc;padding:2px 0;">- ' + p + '</div>';
      });
      html += '</div>';
    });
    html += '<div class="quick-actions"><button onclick="clearChat()">Back</button></div>';
    document.getElementById('analysisContent').innerHTML = html;
  });
}

function searchKB() {
  var query = prompt('[Search Knowledge Base] Enter keyword to search principles/techniques/advice:');
  if (!query) return;
  showLoading();
  fetch('/search?q=' + encodeURIComponent(query))
    .then(function(r){return r.json();})
    .then(function(data){
      var html = '<div class="section"><div class="section-title">[SEARCH: ' + query + ']</div>' +
        '<p style="font-size:11px;color:var(--text2);margin-bottom:8px;">Found ' + (data.results ? data.results.length : 0) + ' matches</p>';
      if (data.results && data.results.length > 0) {
        data.results.forEach(function(bk){
          var typeClass = bk.type === 'principle' ? 'type-principle' : bk.type === 'advice' ? 'type-advice' : 'type-framework';
          var typeLabel = bk.type === 'principle' ? 'Principle' : bk.type === 'advice' ? 'Advice' : 'Framework';
          html += '<div class="book-item">' +
            '<span class="type-badge ' + typeClass + '">' + typeLabel + '</span>' +
            '<div><div>' + bk.text + '</div><div class="src">Source: ' + bk.source + '</div></div>' +
          '</div>';
        });
      } else {
        html += '<p style="font-size:12px;color:var(--text2);">No results found. Try a different keyword.</p>';
      }
      html += '<div class="quick-actions"><button onclick="if(lastAnalysis)renderAnalysis(lastAnalysis)">Back to Analysis</button></div>';
      document.getElementById('analysisContent').innerHTML = html;
    });
}
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path == '/':
            ui_path = Path(__file__).parent / "tjc-ui.html"
            if ui_path.exists():
        ...
        if path == '/search':
            q = params.get('q', [''])[0]
            results = search_knowledge(q, top_n=15) if q else []
    ...
    def do_POST(self):
        ...
        if path == '/analyze' or path == '/analyze/':
            ...
            result = analyze_message(msg, context, background)
            self._json(result)
            return
        if path == '/api-config':
            content_len = int(self.headers.get('Content-Length', 0))
            ...
            self._json(update_ai_config(data))
            return
        self.send_error(404)
...
def main():
    ...
    print(f"\n{'='*50}")
    print(f"  Deep Love Master - Chat Assistant")
    print(f"  Knowledge base: 200+ relationship classics")
    ...
    try:
        ...
        print("\nGoodbye. Remember: authenticity is the highest form of game.\n")
